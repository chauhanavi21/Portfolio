...
const Projects = () => {
    ...
    <section id="projects" className="px-6 py-20 text-center min-h-screen snap-start">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 max-w-[1500px] mx-auto">
        {projects.map((project, index) => (
          <motion.div
            key={index}
            className="bg-[#2d2d2d] rounded-lg shadow-lg overflow-hidden flex flex-col"
  ...
  <h3 className="text-xl font-semibold mb-4 text-center sm:text-left">{project.title}</h3>
  <div>
    <a
      href={project.github}
      target="_blank"
      rel="noopener noreferrer"
      className="border px-4 py-2 rounded hover:bg-white hover:text-black transition inline-block mx-auto sm:ml-0"
    >
      GitHub
    </a>
  </div>
