...
const Skills = () => {
  return (
    <section id="skills" className="px-6 py-20 text-center min-h-screen snap-start bg-[#fff7f9] text-gray-900">
      <h2 className="text-3xl font-bold mb-10 text-[#e85a85]">Skills</h2>

      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 text-left max-w-6xl mx-auto">
        {skills.map((group, i) => (
          <motion.div
            key={i}
            className="bg-white/90 p-5 rounded-lg shadow-md border border-[#f5eaf5]"
            initial={{ opacity: 0, x: i % 2 === 0 ? -50 : 50 }}
            whileInView={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.5 }}
          >
            <h4 className="font-semibold text-lg mb-2 text-[#e85a85]">{group.title}</h4>
            <p className="text-gray-600 text-sm">{group.skills}</p>
          </motion.div>
        ))}
      </div>
    </section>
